		...
		for(int index=0; index < u.size(); ++index){
			for (int iy = 1; iy < ny_all-1; ++iy){
				for (int ix = 1; ix < nx_all-1; ++ix) {		
					...
					real du1 = u[index][iy*nx_all+ix] - u[index][iy*nx_all+(ix-1)];         // Difference to left (u0 - um)
					real du2 = u[index][iy*nx_all+(ix+1)] - u[index][iy*nx_all+ix];          // Difference to right
					real duc = 0.5*(du1+du2); // Centered difference
					du[index][iy*nx_all+ix]= xmin( theta*xmin(du1, du2), duc ); // MAG: might want to inline this?
		...
		for(int index=0; index < u.size(); ++index){
			for (int iy = 1; iy < ny_all-1; ++iy){
				for (int ix = 1; ix < nx_all-1; ++ix) {		
					...
					real du1 = u[index][iy*nx_all+ix] - u[index][(iy-1)*nx_all+ix];         // Difference to left (u0 - um)
					real du2 = u[index][(iy+1)*nx_all+ix] - u[index][iy*nx_all+ix];          // Difference to right
					real duc = 0.5*(du1+du2); // Centered difference
					du[index][iy*nx_all+ix]= xmin( theta*xmin(du1, du2), duc ); // MAG: might want to inline this?
